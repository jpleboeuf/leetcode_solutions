...
mysqlsh --user=jpl --database=leetcode --sql --json --execute="CHECKSUM TABLE Example1, Solution;" | jq --slurp "last.rows | map(select((.Table == \"leetcode.example1\" or .Table ==\"leetcode.solution\")).Checksum) | length == 2"
